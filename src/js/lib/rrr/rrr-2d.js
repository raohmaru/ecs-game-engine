...
export default class Canvas2DRenderer {
	...
	resize(width, height) {
		...
		this._canvas.style.width = `${ width * this._scaleFactor.x | 0 }px`;
		this._canvas.style.height = `${ height * this._scaleFactor.y | 0 }px`;
	}
	...
	drawSprite(sprite, viewport) {
		if(sprite.view) {
			...
			if(intersect.area) {
				...
				if(sprite.transform.isModified()) {
					const spMatrix = sprite.transform.matrix;
					...
					if(sprite.transform.rotation !== 0) {
						...
						d.x = 0;
						d.y = 0;
						this._ctx.rotate(sprite.transform.rotation);
						// const angle_sine = Math.sin(sprite.transform.rotation);
						// const angle_cosine = Math.cos(sprite.transform.rotation);
